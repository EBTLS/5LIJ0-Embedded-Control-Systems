...
DR_CS.phi_aug = DR_CS.sysd.a; 
DR_CS.Gamma_aug = DR_CS.sysd.b; 
DR_CS.Cd = DR_CS.sysd.c;

% with delay
% DR_CS.Gamma0_aug = inv(DR_CS.A) * (expm(DR_CS.A * (DR.h - tau)) - expm(DR_CS.A*0)) * DR_CS.B;
% DR_CS.Gamma1_aug = inv(DR_CS.A) * (expm(DR_CS.A * DR.h) - expm(DR_CS.A * (DR.h-tau))) * DR_CS.B;
% simpilfied calculation
DR_CS.Gamma0_aug = tau * DR_CS.B + 0.5 * DR_CS.A * (DR.h^2 - (DR.h - tau)^2) * DR_CS.B;
DR_CS.Gamma1_aug = (DR.h - tau) * DR_CS.B + 0.5 * DR_CS.A * (DR.h - tau)^2 * DR_CS.B;

% augmentation
DR_CS.phi_aug= [DR_CS.phi_aug DR_CS.Gamma1_aug; zeros(1,DR.nx)];
DR_CS.Gamma_aug= [DR_CS.Gamma0_aug; 1];
...
% with delay
DCM_CS.Gamma0_aug = (DCM_CS.A) \ (expm(DCM_CS.A * (DCM.h-tau))-expm(DCM_CS.A * 0)) * DCM_CS.B;
DCM_CS.Gamma1_aug = (DCM_CS.A) \ (expm(DCM_CS.A * DCM.h)-expm(DCM_CS.A * (DCM.h - tau))) * DCM_CS.B;
...
% Simulink Simulation
assignment1_2022_Simulink_init_Dualrotary(tau, DR.h, DR_CS.K, 2*DR_CS.F)
assignment1_2022_Simulink_init_DCmotor(tau, DCM.h, DCM_CS.K, DCM_CS.F)
